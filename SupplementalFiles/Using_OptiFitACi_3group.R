# OptiFitACi test script -- 3 grouping variables ===============================
#
# This is an example R script (commands) for the use of function 'fitacis4' in
# the OptiFitACi library when THREE input variables (columns) are used to
# identify each experiment. In the test case those variables are 'Temperature'
# (with values of 15, 20, 25, 30, 35, 40 degrees Celsius), 'Plant' (with the
# single value 'Sicot'), and 'Rep' (with values of 1, 2, 3, 4, 5). All
# combinations of Temperature_Plant_Rep exist in the test data and the
# identifier (ID) created is of the form 15_Sicot_1 for 15 degrees, Sicot, rep 1
#
# The leaf CO2 exchange dataset generated by (derived from) the Li-Cor is the
# file:  "TestCotton_3group.csv", with '3group' referring to use of three
# grouping variables to identify each A-Ci experiment/dataset
#
# To use this program/script, the input test dataset should be copied to the
# Windows directory 'C:/Test/', creating
#
#    "C:/Test/TestCotton_3group.csv"
#
...
#
# The 'testCotton_3group.csv' file can be downloaded from:
#
...
# NOTE that the first instruction below will clears all objects from the R
# workspace so if you other things in the workspace you'd like to retain, skip
# this line
...
# Install OptiFitACi package. If "jsamthor/OptiFitACi" has previously been
...
# combination, for a total of 300 A-Ci combinations. The created variable ID
# is unique for each plant-temperature combination (each A-Ci curve fit). The
# grouping variable 'Plant' (meaning Sicot in this case) is the same for each
# experiment but 'Plant' could be different species (for example) in different
# experiments.
#
# This test session will concatenate these factors in the order Temperature,
# Plant, Rep and put an underscore '_' between each factor in the ID variable
# (e.g., 40_Sicot_3). This means when we call fitacis4 that these arguments
# are passed: group1 = 'Temperature', group2 = 'Plant', group3 = 'Rep'. If
# there are only two grouping variables in a different dataset, simply ignore
# all references to 'group3' -- and do not include group3 in the list of
# variables used in the 'fits1 = fitacis4 (...' multi-line command below

  testd$Plant       = as.factor(testd$Plant)
  testd$Temperature = as.factor(testd$Temperature)
  testd$Rep         = as.factor(testd$Rep)

# NOTE that the value of the categorical variable Temperature above is the
# 'nominal' temperature used in the experiment; the actual measured Tleaf is
# used by the model
...
# written to a text (.csv) file. 'acisummary' is a utility function in
# 'plantecowrap'. In the case below the output is written to the file
# 'c:/Test/OptiFitACi_test_kinetics_out_group3.csv', which is an
# implicit concatenation of the working directory (from 'setwd' above) and the
# filename 'OptiFitACi_test_kinetics_out_group3.csv' defined with 'file =' in
# the 'write.csv' argument list below (the 'fits1' in 'fits = fits1' below is
# from 'fits1 = fitacis4 (testd, ...' above

  Opti_out = acisummary(data = testd, fits = fits1,
                        group1 = "Temperature", group2 = "Plant", group3 = "Rep")
  write.csv(Opti_out, file = 'OptiFitACi_test_kinetics_out_3group.csv',
            row.names = F)

# Now open (create) a file ('OptiFitACi_test_plots_3group.pdf') to contain plots
...
# will be written to the "c:/Test" directory/folder
  ...
  pdf("OptiFitACi_test_plots_3group.pdf")  ;  par(mar=c(5.1,5.1,4.1,2.1))
...
# this case, but is the correct way to refer to a list)
...
# "C:\Test\OptiFitACi_test_plots_3group.pdf" and
# "C:\Test\OptiFitACi_test_kinetics_out_3group.csv"
